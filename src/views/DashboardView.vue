...
const todayLabel = parseLocalISODate(today).toLocaleDateString('nl-BE', {
  weekday: 'long',
  day: 'numeric',
  month: 'long',
  year: 'numeric',
...
const pendingCount = computed(
  () => shiftsStore.shifts.filter((s) => s.status === 'pending').length,
);

/**
 * Real admin-side numbers — was hardcoded mock data, see the FRD/duplication
 * audit. The admin/manager subscription is bounded to exactly this week (see
 * the watch below), so every counter on this page is a "deze week" number.
 */
const currentWeekStart = computed(() => weekStartFor(today));
const weekWindow = computed(() => ({
  from: currentWeekStart.value,
  toExclusive: addDaysISO(currentWeekStart.value, 7),
}));
const weekShifts = computed(() =>
  shiftsStore.shifts.filter(
    (s) => s.date >= weekWindow.value.from && s.date < weekWindow.value.toExclusive,
  ),
);
...
const todaysTeamLeaderCount = computed(
  () =>
    new Set(
      todaysShifts.value.filter((s) => s.employeeIsTeamLeader).map((s) => s.assignedEmployeeId),
    ).size,
);
const weekApprovedCount = computed(
  () => weekShifts.value.filter((s) => s.status === 'approved').length,
);
...
const weeklyStaffing = computed(() => {
  ...
  return days.map(([date, d]) => ({
    day: parseLocalISODate(date).toLocaleDateString('nl-BE', { weekday: 'short', day: 'numeric' }),
    total: d.total,
    leaders: d.leaders,
...
const recruitmentPulse = computed(() => {
  ...
  return [
    { label: 'Nieuwe leads', value: c.new, color: 'bg-[#111]', pct: (c.new / total) * 100 },
    {
      ...
      pct: (c.interviewPlanned / total) * 100,
    },
    {
      label: 'Opgekomen',
      value: c.attended,
...
watch(
  ...
  (id) => {
    ...
    if (isMember.value) {
      ...
      // Current week only — the dashboard never shows anything older, and an
      // unbounded office-wide read is a 50k/day Spark-quota risk.
      shiftsStore.subscribe(id, weekWindow.value);
...
<template>
  ...
  <div v-else class="mx-auto max-w-7xl space-y-8">
    <section class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div>
        <p class="text-sm capitalize text-neutral-mute">
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <article class="min-h-32 border border-black/5 bg-white p-5">
        ...
        <p class="mt-1 text-xs opacity-70">
          {{ recruitmentStore.funnelCounts.interviewPlanned }} sollicitaties gepland
        </p>
      </article>
      <article class="min-h-32 border border-black/5 bg-primary-pink p-5 text-white">
        ...
        <p class="mt-4 text-3xl font-bold">{{ pendingCount }}</p>
        <p class="mt-1 text-xs opacity-70">Deze week nog te beoordelen</p>
      </article>
    </section>
    <section class="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
      <article class="border border-black/5 bg-white p-6">
        ...
        >
          ...
          >
            <span class="text-xs font-bold">{{ day.total }}</span>
            <div
            ...
            <span class="text-[11px] text-neutral-mute">{{ day.day }}</span>
          </div>
        </div>
