...
const visibleLeads = computed(() =>
  selectedStage.value === 'all'
    ? store.leads
    : store.leads.filter((l) => l.stage === selectedStage.value),
);
...
async function moveStage(leadId: string, stage: LeadStage): Promise<void> {
  ...
  if (ok && lead && auth.user.value) {
    void auditLog.record(
      ...
      lead.name,
      `${fromStage ? LEAD_STAGE_LABELS[fromStage] : '?'} → ${LEAD_STAGE_LABELS[stage]}`,
    );
  }
}
...
<template>
  <div class="mx-auto max-w-7xl space-y-6">
    <section class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
      <div>
        <p class="text-sm text-neutral-mute">
          {{ store.leadsThisWeek }} nieuwe leads deze week · Kantoor {{ officeLabel(officeId) }}
        </p>
    ...
    <section v-if="canManage" class="grid gap-6 xl:grid-cols-[1fr_1.4fr]">
      <div class="grid grid-cols-3 gap-4">
        <article class="border border-black/5 bg-white p-5">
          ...
          <p class="mt-3 text-3xl font-bold text-emerald-600">
            {{ store.qualityStats.attendanceRate }}%
          </p>
        </article>
        <article class="border border-black/5 bg-white p-5">
          <p class="text-xs uppercase tracking-[0.16em] text-neutral-mute">No-show</p>
          <p class="mt-3 text-3xl font-bold text-semantic-danger">
            {{ store.qualityStats.noShowRate }}%
          </p>
        </article>
        <article class="border border-black/5 bg-white p-5">
      ...
      <div class="border border-black/5 bg-white p-5">
        ...
        <div class="mt-4 space-y-3">
          <div
            v-for="row in store.bySourcePerformance"
            :key="row.source"
          ...
          >
            ...
            <span class="w-20 shrink-0 text-right text-neutral-mute"
              >{{ row.hired }}/{{ row.total }} aangenomen</span
            >
          </div>
    ...
    <section class="overflow-x-auto border border-black/5 bg-white">
      ...
      <p v-else-if="!visibleLeads.length" class="p-8 text-center text-sm text-neutral-mute">
        Nog geen leads.
      </p>
    </section>
