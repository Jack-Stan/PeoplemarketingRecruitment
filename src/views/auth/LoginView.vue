    ...
    <section class="relative w-full max-w-sm overflow-hidden rounded-lg bg-neutral-white shadow-md">
      ...
      <div class="border-t-2 border-primary-pink p-8">
        <form v-if="!isResetMode" class="space-y-4" @submit.prevent="onSubmit">
          ...
          <label
            v-if="email && password"
            class="flex cursor-pointer select-none items-center gap-2 text-sm text-neutral-ink"
          >
            <input
              v-model="rememberMe"
              type="checkbox"
          ...
          <p class="text-center text-xs">
            <button
              type="button"
              class="font-semibold text-primary-pink hover:underline"
              @click="openReset"
            >
              Wachtwoord vergeten?
            </button>
        ...
        <form v-else class="space-y-4" @submit.prevent="onSendReset">
          ...
          <p class="text-center text-xs">
            <button
              type="button"
              class="font-semibold text-neutral-mute hover:underline"
