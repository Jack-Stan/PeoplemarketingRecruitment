...
const links = computed(() => [
  ...
  ...(auth.hasRole('TeamMember')
    ? [{ label: 'Mijn planning', to: '/mijn-planning', icon: '▣' }]
    : []),
  ...(auth.hasRole('Administrator', 'TeamManager')
    ? [{ label: 'Planning', to: '/planning', icon: '▣' }]
    : []),
  ...(auth.hasRole('Administrator', 'TeamManager')
    ? [{ label: 'Medewerkers', to: '/employees', icon: '♙' }]
    : []),
...
const accountLabel = computed(
  () => auth.displayName.value || auth.user.value?.email || 'Administrator',
);
  ...
  <div class="min-h-screen bg-[#f6f5f3] text-neutral-ink">
    <aside class="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col bg-[#111111] text-white lg:flex">
      <div class="border-b border-white/10 px-7 py-6">
        <img :src="logoUrl" :alt="brand.logo.alt" class="h-8 w-auto" />
        <p class="mt-2 text-[10px] uppercase tracking-[0.2em] text-white/40">Operationeel</p>
      </div>
      <div v-if="isAdmin && offices.length > 1" class="border-b border-white/10 px-5 py-4">
        <label class="block text-[10px] font-bold uppercase tracking-[0.2em] text-white/35"
          >Kantoor</label
        >
        <select
      ...
      <nav class="flex-1 space-y-1 px-3 py-7">
        ...
        <RouterLink
          ...
          :key="link.to"
          :to="link.to"
          class="flex items-center gap-3 border-l-2 px-4 py-3 text-sm font-medium text-white/60 hover:bg-white/5 hover:text-white"
          :class="
      ...
      <div ref="accountMenuRef" class="relative border-t border-white/10 p-4">
        <div
          ...
          class="absolute inset-x-4 bottom-full mb-2 border border-white/10 bg-[#1a1a1a] py-1 shadow-lg"
        >
          <RouterLink
            to="/settings"
    ...
    <div class="lg:pl-64">
      <nav
        class="flex items-center justify-between border-b border-black/5 bg-[#111111] px-5 py-2 lg:hidden"
      >
        ...
        <div ref="mobileMenuRef" class="relative">
          ...
          >
            ...
            <RouterLink
              ...
              :class="
                isLinkActive(link.to)
                  ? 'bg-primary-pink/5 text-primary-pink'
                  : 'text-neutral-ink hover:bg-[#faf9f7]'
              "
            >
            ...
            <template v-if="adminLinks.length">
              <p
                class="border-t border-black/5 px-4 pb-1 pt-2 text-[10px] font-bold uppercase tracking-[0.16em] text-neutral-mute"
              >
                Beheer
              </p>
              <RouterLink
